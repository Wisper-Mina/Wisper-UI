...
import { generateProof, generateProofWithPreviousProof } from '../proof/generateProof';
...
describe('Chat functions', () => {
    ...
    beforeAll(async () => {
        ...
        const sharedSecret = CryptoUtils.computeSharedSecret(encryptionPrivKey1, encryptionPubKey2);
        ...
        const proof = await generateProof(signingPublicKey1, messageHash, messageSignature, merkleTree, messageIndex);
    ...
    it('should decrypt and get exact message', async () => {
        const sharedSecret = CryptoUtils.computeSharedSecret(encryptionPrivKey2, encryptionPubKey1);
        const sharedKey = CryptoUtils.fieldToBuffer(sharedSecret);
    ...
    it("should other client verify and keep chatting as well", async () => {
        ...
        const message = newPureMessage.split('').map(char => Field(char.charCodeAt(0)));
        const messageHash = Poseidon.hash(message);
        const messageSignature = Signature.create(
        signingPrivateKey2,
            messageHash.toFields()
        ...
        const newProof = await generateProofWithPreviousProof(signingPublicKey2, messageHash, messageSignature, merkleTree, messageIndex, messagePackage.proof);
        messagePackage2 = { encryptedMessage: encryptedMessage, proof: newProof };
    })
    it('should decrypt and get exact message', async () => {
        const sharedSecret = CryptoUtils.computeSharedSecret(encryptionPrivKey1, encryptionPubKey2);
        const sharedKey = CryptoUtils.fieldToBuffer(sharedSecret);
        const decryptedMessage = decrypt(sharedKey, messagePackage2.encryptedMessage.iv, messagePackage2.encryptedMessage.encryptedData, messagePackage2.encryptedMessage.authTag);

        expect(newPureMessage).toStrictEqual(decryptedMessage);
    })
    it('should verify the proof', async () => {
        const isValid = await MessageVerificationProgram.verify(messagePackage2.proof);
        expect(isValid).toBe(true);
    })
});
